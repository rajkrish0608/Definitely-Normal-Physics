...
func _ready() -> void:
	...
	var level_scene := LevelLoader.load_level("res://levels/json/world_01_level_01.json")
	if level_scene:
		add_child(level_scene)
		
		# Create simple player for testing (placeholder until Player.tscn is ready)
		var player := _create_test_player()
		var spawn := level_scene.get_node_or_null("PlayerSpawn")
		if spawn:
...
func _create_test_player() -> CharacterBody2D:
	...
	var script := load("res://scripts/player/PlayerController.gd")
